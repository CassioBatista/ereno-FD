...
class Grasp:
    """Abstract GRASP base — mirrors Grasp.java."""

    criteria_metric: str = "F1SCORE"

    def __init__(self):
        self.max_time: int = 24 * 60 * 60 * 1000  # ms
        ...
        self._all_instances = None  # (X, y) tuple
    ...
    def construct_solution(self, full_rcl: list[int]) -> GraspSolution:
        ...
        while len(solution.get_array_features()) < self.num_features and rcl_copy:
            idx = random.randrange(len(rcl_copy))
            solution.add_feature(rcl_copy.pop(idx))
        for f in rcl_copy:
    ...
    def reconstruct_solution(self, initial: GraspSolution) -> GraspSolution:
        ...
        while len(solution.get_array_features()) < self.num_features and full_rcl:
            idx = random.randrange(len(full_rcl))
            solution.add_feature(full_rcl.pop(idx))
        for f in full_rcl:
            solution.add_feature_flip(f)
    ...
    def avaliar(self, solution: GraspSolution) -> GraspSolution:
        ...
        if config.DEBUG_MODE:
            ...
            print(f"Classifier: {config.SINGLE_CLASSIFIER_MODE.get_classifier_name() if config.SINGLE_CLASSIFIER_MODE else 'None'}")
            print(f"Folds: {config.FOLDS}")
            print(f"Seed: {config.GRASP_SEED}")
        ...
        if config.CROSS_VALIDATION:
            ...
            if config.TRAIN is None or config.TEST is None:
                print("ERROR: TRAIN and TEST must be set when CROSS_VALIDATION=False", flush=True)
            else:
                from python.evaluation import run_single_classifier
                from python.util import filter_features
                X_tr, y_tr = filter_features(config.TRAIN[0], config.FEATURE_SELECTION), config.TRAIN[1]
                X_te, y_te = filter_features(config.TEST[0], config.FEATURE_SELECTION), config.TEST[1]
                solution.evaluation = run_single_classifier(X_tr, y_tr, X_te, y_te)

        self.number_evaluation += 1
        ev_str = (
            f"EV;{self.number_evaluation};{solution.evaluation.f1score};"
            f"{solution.get_array_features()}"
        )
        print(ev_str)

        if self.output_manager:
            self.output_manager.write_detail(solution)
