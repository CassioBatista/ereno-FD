...
def run_single_classifier(X: np.ndarray, y: np.ndarray,
                          total_folds: int, seed: int) -> list[Result]:
    skf = StratifiedKFold(n_splits=total_folds, shuffle=True, random_state=seed)
    results: list[Result] = []
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        X_train, X_test = X[train_idx], X[test_idx]
        y_train, y_test = y[train_idx], y[test_idx]
        r = evaluation.run_single_classifier(X_train, y_train, X_test, y_test)
        results.append(r)
...
def setup_and_run(total_folds: int, seed: int, classifier_ext, fs: list[int],
                  X: np.ndarray | None = None, y: np.ndarray | None = None) -> list[Result]:
...
def run_with_instances(X: np.ndarray, y: np.ndarray, features: list[int],
                       classifier_ext, print_confusion: bool = False) -> None:
    print(classifier_ext.get_classifier_name() + ";", end="")
...
def print_folds(X: np.ndarray, y: np.ndarray, features: list[int] | None = None,
    ...
    for clf in all_custom:
        ...
        fold_results = setup_and_run(config.FOLDS, config.EVALUATION_SEED, clf, fs, X, y)
        all_results.append(fold_results)

    if print_summary and not config.SINGLE_FOLD_MODE:
        for clf, folds in zip(all_custom, all_results):
            print(clf.get_classifier_name() + ";", end="")
            for r in folds:
